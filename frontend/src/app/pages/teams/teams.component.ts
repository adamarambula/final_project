...
export class TeamsComponent {
  ...
  fetchTeams() {
    ...
    // For non admin - gets teams that user belongs to
    if(!this.companyId){
      this.teams = this.authService.getCurrentUser()?.teams  || []
    }
    
    // For admin - gets all teams for a company
    this.companyService.getCompanyId().subscribe(companyId => {
      console.log('companyid from teams', companyId)
      if (companyId) {
