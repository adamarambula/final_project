...
import { Project, ProjectData, Team, UpdateProjectData } from 'src/app/shared/models';
    ...
    if (this.newProject && this.newProject.name && this.newProject.description) {
      ...
      .subscribe({
        next: (response: any) => {
          ...
          this.saveProjectId = response.id;
...
updatedProjectSubmitted(updatedProjectData: UpdateProjectData, project: Project) {
  ...
  if(this.isAdmin){
    console.log("etner in the consoel log statemet")
